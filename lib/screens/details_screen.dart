...
import 'package:freaking/screens/payment_methods_screen.dart';
...
/// Pantalla de detalles de la aplicación.
///
/// Esta clase contiene la interfaz de la pantalla que se muestra después de presionar el botón de detalles en el menú inferior de la aplicación.
/// En esta pantalla aparecen tres botones que llevan a las pantallas de "Métodos de pago", "Mis pedidos" e "Información y asistencia al cliente".
...
class _DetailsScreenState extends State<DetailsScreen> {
  ...
  void initState() {
    ...
    super.initState();
    debugPrint('Pantalla DetailsScreen seleccionada');
    _loadUserName();
  }

  /// Método para cargar el nombre de usuario desde SharedPreferences.
  Future<void> _loadUserName() async {
    debugPrint('_loadUserName: iniciando lectura de SharedPreferences');
    final prefs = await SharedPreferences.getInstance();
    final userName = prefs.getString('name') ?? '';
    debugPrint('_loadUserName: nombre obtenido="$userName"');
    setState(() {
      userNameController.text = userName;
      debugPrint('_loadUserName: actualizando campo userNameController');
    });
  }

  /// Método para guardar el nuevo nombre en SharedPreferences.
  Future<void> saveNameToPreferences(String newName) async {
    debugPrint('saveNameToPreferences: guardando nombre="$newName"');
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('name', newName);
    debugPrint('saveNameToPreferences: nombre guardado en SharedPreferences');
  }

  /// Método para limpiar los datos del usuario en SharedPreferences.
  Future<void> clearUserData() async {
    debugPrint('clearUserData: limpiando datos de SharedPreferences');
    final prefs = await SharedPreferences.getInstance();

    /// Eliminar el contenido de SharedPreferences.
    await prefs.clear();
    prefs.setBool("onboarding", true);
    debugPrint('clearUserData: SharedPreferences limpiado, onboarding=true');
  ...
  Future<void> logout(BuildContext context) async {
    try {
      debugPrint('logout: iniciando cierre de sesión FirebaseAuth');

      /// Cerrar sesión de Firebase Auth.
      await FirebaseAuth.instance.signOut();
      debugPrint('logout: sesión de FirebaseAuth cerrada');
      ...
      debugPrint('logout: navegado a LoginScreen');
    } catch (e) {
  ...
  Future<Map<String, String?>> getCustomerData() async {
    debugPrint('getCustomerData: leyendo SharedPreferences');
    final prefs = await SharedPreferences.getInstance();
    String? stripeCustomerId = prefs.getString('stripeCustomerId');
    String? name = prefs.getString('name');
    debugPrint(
        'getCustomerData: stripeCustomerId=$stripeCustomerId, name=$name');
    return {
  ...
  Future<void> updateCustomerNameInFirebase(String newName) async {
    debugPrint('updateCustomerNameInFirebase: nuevo nombre="$newName"');
    ...
    try {
      debugPrint('updateCustomerNameInFirebase: preparando solicitud HTTP');
      ...
      debugPrint(
          'updateCustomerNameInFirebase: respuesta HTTP ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          debugPrint(
              'updateCustomerNameInFirebase: userNameController actualizado');
        });
      } else {
  ...
  Future<bool> checkPendingOrders() async {
    try {
      debugPrint('checkPendingOrders: verificando pedidos pendientes');
      ...
      final userOrderSnapshot = await FirebaseFirestore.instance
          ...
          .get();
      debugPrint(
          'checkPendingOrders: documentos obtenidos ${userOrderSnapshot.docs.length}');
  ...
  Future<void> requestAccountDeletion(BuildContext context) async {
    try {
      debugPrint('requestAccountDeletion: iniciando proceso de eliminación');
      ...
      debugPrint('requestAccountDeletion: idToken obtenido');
      ...
      debugPrint(
          'requestAccountDeletion: respuesta HTTP ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success']) {
          ...
          debugPrint('requestAccountDeletion: navegando a RegisterScreen');
        } else {
          print(
              'Error al solicitar la eliminación de la cuenta: ${responseData['message']}');
  ...
  void showErrorNotification(BuildContext context) {
    debugPrint('showErrorNotification: mostrando diálogo de error');
    showDialog(
      context: context,
  ...
  Future<void> _showExitDialog(BuildContext context) async {
    debugPrint('_showExitDialog: mostrando diálogo de confirmación de salida');
    ...
    if (exitApp ?? false) {
      ...
      setState(() {
        _canPop = true;
        debugPrint('_showExitDialog: usuario confirmó salida, _canPop=true');
      });
      SystemNavigator.pop(); // Cerrar la aplicación.
      debugPrint('_showExitDialog: SystemNavigator.pop invocado');
    } else {
      debugPrint('_showExitDialog: usuario canceló salida');
    }
  }

  /// Widget que construye la interfaz de la pantalla de detalles de la aplicación.
  ///
  /// Este widget muestra la pantalla de detalles después de presionar el botón de detalles en el menú inferior de la aplicación.
  ...
  Widget build(BuildContext context) {
    debugPrint('build: construyendo DetailsScreen');
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, result) async {
        if (didPop) {
          debugPrint('build: PopScope didPop=true, no mostrar diálogo');
          return;
        }
                ...
                children: [
                  ...
                  Positioned(
                    ...
                    child: CustomButton.customCircularElevatedButtonWithIcon(
                      onPressed: () {
                        debugPrint(
                            'build: Botón carrito presionado, navegando a CartScreen');
                  ...
                  Positioned(
                    ...
                    child: CustomButton.customCircularElevatedButtonWithIcon(
                      onPressed: () {
                        debugPrint(
                            'build: Botón perfil presionado, mostrando diálogo de perfil');
                            ...
                            return AlertDialog(
                                  ...
                                  child: ListBody(
                                    children: <Widget>[
                                      ...
                                      CustomButton.customElevatedButtonWithText(
                                        onPressed: () async {
                                          debugPrint(
                                              'Diálogo perfil: Guardar modificaciones presionado con nombre="${userNameController.text}"');
                                          await saveNameToPreferences(
                                              userNameController.text);

                                          /// Actualiza el nombre en Firebase y Stripe.
                                          await updateCustomerNameInFirebase(
                                              userNameController.text);
                                      ...
                                      CustomButton.customElevatedButtonWithText(
                                        onPressed: () {
                                          debugPrint(
                                              'Diálogo perfil: Botón Cerrar sesión presionado, mostrando confirmación');
                                              ...
                                              return AlertDialog(
                                                    ...
                                                    child: ListBody(
                                                      children: <Widget>[
                                                        ...
                                                        CustomButton
                                                          ...
                                                          onPressed: () async {
                                                            debugPrint(
                                                                'Diálogo cierre sesión: Confirmado');
                                                            await logout(
                                                                context);
                                                            ScaffoldMessenger
                                                                    .of(context)
                                                ...
                                                actions: <Widget>[
                                                  TextButton(
                                                    ...
                                                    onPressed: () {
                                                      debugPrint(
                                                          'Diálogo cierre sesión: Cancelado');
                                      ...
                                      CustomButton.customElevatedButtonWithText(
                                        onPressed: () {
                                          debugPrint(
                                              'Diálogo perfil: Botón Eliminar cuenta presionado, mostrando confirmación');
                                              ...
                                              return AlertDialog(
                                                    ...
                                                    child: ListBody(
                                                      children: <Widget>[
                                                        ...
                                                        CustomButton
                                                          ...
                                                          onPressed: () async {
                                                            debugPrint(
                                                                'Diálogo eliminar cuenta: Confirmado');
                                                ...
                                                actions: <Widget>[
                                                  TextButton(
                                                    ...
                                                    onPressed: () {
                                                      debugPrint(
                                                          'Diálogo eliminar cuenta: Cancelado');
                                                      Navigator.of(context)
                                                          .pop();
                                                    },
                                                  ),
                              ...
                              actions: <Widget>[
                                TextButton(
                                  ...
                                  onPressed: () {
                                    debugPrint(
                                        'Diálogo perfil principal: cerrado');
...
/*                Positioned(
                  ...
                  left: 0,
                  right: 0,
                  child: CustomContainer().detailsScreenHeader(context),
                ),*/
                  Positioned(
                      top: 210,
                      left: 0,
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          children: [
                            Center(
                              child: CustomButton
                                  .customElevatedDetailsScreenButton(
                                onPressed: () async {
                                  debugPrint(
                                      'Botón “Métodos de pago” presionado');
                                ...
                                customTextStyle: CustomTextStyle
                                    .dynamicColorSemiBold18WithShadow(context),
                                buttonHeight: 120,
                                buttonWidth: 295,
                                icon: SvgPicture.asset(
                                    'assets/svg/icon/CreditCard.svg'),
                                topLeftIcon: SvgPicture.asset(
                                    'assets/svg/icon/TopLeftCorner.svg'),
                                topRightIcon: SvgPicture.asset(
                                    'assets/svg/icon/TopRightCorner.svg'),
                                bottomLeftIcon: SvgPicture.asset(
                                    'assets/svg/icon/BottomLeftCorner.svg'),
                                bottomRightIcon: SvgPicture.asset(
                                    'assets/svg/icon/BottomRightCorner.svg'),
                              ),
                            ),
                            ...
                            Center(
                              child: CustomButton
                                  .customElevatedDetailsScreenButton(
                                onPressed: () {
                                  debugPrint('Botón “Mis pedidos” presionado');
                                  // Lógica para la segunda "pestaña"
/*                                CustomNavigator.instantNavigation(
                            ...
                            Center(
                              child: CustomButton
                                ...
                                onPressed: () {
                                  debugPrint(
                                      'Botón “Información y asistencia al cliente” presionado');
                                  // Lógica para la segunda "pestaña"
/*                                CustomNavigator.instantNavigation(
                                    context, InfoAndAttendanceScreen());*/
                                },
                                text:
